...
static int recur_count(link node)
{
    if(node == NULL)
        return 0;

    return 1 + recur_count(node->next);
}

static void recur_traverse(link node, void (*visit)(link))
{
    if (node == NULL)
        return;

    (*visit)(node);
    recur_traverse(node->next, visit);
}

static void recur_traverseR(link node, void (*visit)(link))
{
    if (node == NULL)
        return;

    recur_traverseR(node->next, visit);
    (*visit)(node);
}
...
void list_traverse(void (*visit)(link))
{
    recur_traverse(head->next, visit);
}

void list_traverseR(void (*visit)(link))
{
    recur_traverseR(head->next, visit);
}
